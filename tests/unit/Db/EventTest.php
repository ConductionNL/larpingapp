...
class EventTest extends TestCase
{
    ...
    {
        $e = new Event(); $e->setTitle('Summer LARP');
        $j = $e->jsonSerialize();
    ...
    public function testJsonSerializeFields(): void
    {
        ...
        foreach (['id','title','description','startDate','endDate','userId'] as $f) self::assertArrayHasKey($f, $j);
    ...
    {
        $e = new Event(); $e->hydrate(['title'=>'W','description'=>'C','userId'=>'admin']);
        $j = $e->jsonSerialize();
        self::assertSame('W', $j['title']);
        self::assertSame('C', $j['description']);
        self::assertSame('admin', $j['userId']);
    }
    public function testUserIdProperty(): void
    {
        $e = new Event(); $e->setUserId('p1');
        self::assertSame('p1', $e->jsonSerialize()['userId']);
    }
    public function testToArrayMatchesJsonSerialize(): void
    {
        $e = new Event(); $e->setTitle('Test');
        self::assertSame($e->jsonSerialize(), $e->toArray());
    }
    public function testDefaultsAreNull(): void
    {
        $j = (new Event())->jsonSerialize();
        self::assertNull($j['title']); self::assertNull($j['description']);
        self::assertNull($j['startDate']); self::assertNull($j['endDate']); self::assertNull($j['userId']);
    }
}
