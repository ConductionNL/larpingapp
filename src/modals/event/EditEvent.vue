...
<template>
	<NcModal v-if="navigationStore.modal === 'editEvent'" ref="modalRef" @navigationStore="store.setModal(false)">
...
export default {
	...
	methods: {
		async editEvent() {
			this.loading = true
			try {
				await this.eventStore.saveEvent(this.eventStore.eventItem)
